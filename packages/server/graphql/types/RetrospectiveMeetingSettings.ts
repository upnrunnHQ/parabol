  ...
  fields: () => ({
    ...
    reflectTemplates: {
      ...
      resolve: ({teamId}, _args, {dataLoader}) => {
        const templates = dataLoader.get('meetingTemplatesByTeamId').load(teamId)
        return templates.filter((template) => template.type === MeetingTypeEnum.retrospective)
      }
    },
    teamTemplates: {
      type: new GraphQLNonNull(new GraphQLList(new GraphQLNonNull(ReflectTemplate))),
      ...
      resolve: async ({teamId}, _args, {dataLoader}) => {
        ...
        const teamTemplates = templates.filter(
          (template) => template.scope === 'TEAM' && template.type === MeetingTypeEnum.retrospective
        )
    ...
    organizationTemplates: {
      ...
      resolve: async ({teamId}, {first, after}, {dataLoader}) => {
        ...
        const organizationTemplates = templates.filter(
          (template) => template.scope !== 'TEAM' && template.teamId !== teamId && template.type === MeetingTypeEnum.retrospective
        )
